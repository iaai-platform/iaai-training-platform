...
exports.renderLoginPage = (req, res) => {
  ...
  console.log('🔍 Current user:', req.user ? `${req.user.firstName} (${req.user.email})` : 'None');
  ...
  // Get flash messages
  const errorMessage = req.flash('error_message')[0] || req.flash('error')[0] || null;
  const successMessage = req.flash('success_message')[0] || null;
  
  console.log('📨 Messages to display:', { error: errorMessage, success: successMessage });

  res.render('login', { 
    error_message: errorMessage,
    success_message: successMessage,
    user: null,
    process: process // Pass process to access env variables in template
  });
};
...
exports.handleLogin = async (req, res, next) => {
  ...
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      ...
      req.flash('error_message', info ? info.message : 'Invalid email or password');
      return res.redirect('/login');
    }
