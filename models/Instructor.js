...
const instructorSchema = new mongoose.Schema(
  {
    ...
    assignedCourses: [
      {
        ...
        role: {
          type: String,
          enum: ["Lead Instructor", "Co-Instructor", "Assistant"],
    ...
    contractType: {
      type: String,
      enum: ["Full-time", "Part-time", "Contract", "Guest"],
      default: "Contract",
    },
    hourlyRate: { type: Number },
    showOnPublicPage: { type: Boolean, default: true },
...
instructorSchema.methods.updateCourseStatus = function (courseId, newStatus) {
  const course = this.assignedCourses.find(
...
instructorSchema.methods.calculateAverageRating = function () {
  ...
  const sum = this.ratings.courseRatings.reduce(
...
instructorSchema.methods.getUpcomingCourses = function () {
  ...
  return this.assignedCourses.filter(
    (course) => new Date(course.startDate) > now && course.status === "Upcoming"
  );
};
