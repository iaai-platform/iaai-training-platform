...
exports.getOnlineLiveTraining = async (req, res) => {
  try {
    ...
    console.log('👤 User:', user ? `${user.firstName} (${user.email})` : 'Not logged in');
    ...
    for (let course of courses) {
      ...
      const transformedCourse = {
        ...
        coInstructors: course.instructors?.additional?.map(i => i.name).filter(Boolean) || [],
        
        // Images
        images: {
          mainImage: course.media?.mainImage,
          brochures: course.media?.documents?.filter(url => url.includes('brochure')) || [],
          catalogs: course.media?.documents?.filter(url => url.includes('catalog')) || []
        },
        
        // Calculated fields
        enrolledStudents: enrolledCount,
        availableSeats: Math.max(0, (course.enrollment?.seatsAvailable || 50) - enrolledCount),
        isMultiDay: course.schedule?.sessions?.length > 1 || 
                    (course.schedule?.endDate && course.schedule?.startDate && 
                     new Date(course.schedule.endDate).getTime() !== new Date(course.schedule.startDate).getTime())
      };
      
      // Calculate status and other display fields
      const now = new Date();
      const startDate = transformedCourse.startDate ? new Date(transformedCourse.startDate) : null;
      const registrationDeadline = transformedCourse.registrationDeadline || 
                                   (startDate ? new Date(startDate.getTime() - 7 * 24 * 60 * 60 * 1000) : null);
      ...
      transformedCourse.hasEarlyBird = transformedCourse.earlyBirdPrice && 
                                       transformedCourse.earlyBirdPrice < transformedCourse.price;
      
      transformedCourse.mainImageUrl = course.media?.mainImage?.url || 
      ...
      if (startDate && startDate > now) {
        ...
        transformedCourse.timeUntilStart = `${daysUntilStart} day${daysUntilStart !== 1 ? 's' : ''}`;
      } else {
    ...
    transformedCourses.sort((a, b) => {
      if (a.displayStatus === 'In Progress' && b.displayStatus !== 'In Progress') return 1;
      if (a.displayStatus !== 'In Progress' && b.displayStatus === 'In Progress') return -1;
...
exports.getCourseDetails = async (req, res) => {
  try {
    ...
    const courseEndDate = course.schedule?.endDate ? new Date(course.schedule.endDate) : null;
    ...
    const transformedCourse = {
      ...
      isMultiDay: course.schedule?.sessions?.length > 1,
      courseDays: course.schedule?.sessions?.map(session => ({
        date: session.date,
      ...
      instructor: course.instructors?.primary?.instructorId?.fullName || 
      ...
      instructorDetails: course.instructors?.primary?.instructorId ? {
        ...
        profilePicture: course.instructors.primary.instructorId.profilePicture,
        qualifications: course.instructors.primary.instructorId.qualifications || [],
        specializations: course.instructors.primary.instructorId.specializations || [],
        yearsOfExperience: course.instructors.primary.instructorId.yearsOfExperience
      } : null,
      
      // Co-instructors
      coInstructors: course.instructors?.additional?.map(inst => 
        inst.instructorId?.fullName || inst.name
      ).filter(Boolean) || [],
      coInstructorDetails: course.instructors?.additional?.map(inst => 
        inst.instructorId ? {
          _id: inst.instructorId._id,
          fullName: inst.instructorId.fullName,
          title: inst.instructorId.title,
          bio: inst.instructorId.bio,
          profilePicture: inst.instructorId.profilePicture,
          role: inst.role
        } : null
      ).filter(Boolean) || [],
      ...
      mainImage: course.media?.mainImage?.url || '/images/default-online-course.jpg',
      ...
      certificateType: course.certification?.types?.[0] || 'completion',
      ...
      hasEarlyBird: course.enrollment?.earlyBirdPrice && 
    ...
    transformedCourse.availableSeats = Math.max(0, transformedCourse.seatsAvailable - enrolledCount);
    ...
    if (currentDate < courseStartDate) {
      const daysUntilStart = Math.ceil((courseStartDate - currentDate) / (1000 * 60 * 60 * 24));
      const hoursUntilStart = Math.ceil((courseStartDate - currentDate) / (1000 * 60 * 60));
      
      if (hoursUntilStart <= 24) {
        courseStatusMessage = `Course starts in ${hoursUntilStart} hour${hoursUntilStart !== 1 ? 's' : ''}`;
      } else {
        courseStatusMessage = `Course starts in ${daysUntilStart} day${daysUntilStart !== 1 ? 's' : ''}`;
      }
    ...
    if (req.user) {
      ...
      if (userCourse) {
        userRegistrationStatus = {
          ...
          isRegistered: ['paid', 'registered'].includes(userCourse.enrollmentData.status),
          isInCart: userCourse.enrollmentData.status === 'cart',
          isInWishlist: userCourse.enrollmentData.status === 'wishlist',
          attendanceConfirmed: userCourse.userProgress?.overallAttendancePercentage > 0,
          canAccessRecordings: userCourse.userProgress?.overallAttendancePercentage > 0,
    ...
    const assessmentInfo = {
      hasAssessment: transformedCourse.assessmentRequired && transformedCourse.assessmentType !== 'none',
      requiresQuiz: transformedCourse.assessmentType === 'quiz' || transformedCourse.assessmentType === 'both',
      requiresPractical: transformedCourse.assessmentType === 'practical' || transformedCourse.assessmentType === 'both',
    ...
    } else if (transformedCourse.registrationDeadline && new Date(transformedCourse.registrationDeadline) <= currentDate) {
      transformedCourse.status = 'Registration Closed';
    } else {
      transformedCourse.status = 'Open to Register';
    }

    console.log('🎯 Rendering course details page with transformed data');
...
exports.joinLiveSession = async (req, res) => {
  try {
    ...
    const endDate = course.schedule.endDate ? new Date(course.schedule.endDate) : new Date(startDate.getTime() + 24 * 60 * 60 * 1000);
...
exports.getCourseMaterials = async (req, res) => {
  try {
    ...
    const course = await OnlineLiveTraining.findById(courseId, 'materials media basic');
    ...
    const organizedMaterials = {
      ...
      resources: course.media?.links || [],
      totalFiles: (course.materials?.handouts?.length || 0) + 
...
exports.getCourseAssessment = async (req, res) => {
  try {
    ...
    if (userCourse.userProgress.overallAttendancePercentage < 80) {
      return res.status(403).json({ 
        success: false, 
        message: 'You must attend at least 80% of the course before taking the assessment' 
      });
    }
    ...
    if (userCourse.userProgress.bestAssessmentScore >= course.assessment.passingScore) {
      return res.json({
        success: true,
        alreadyPassed: true,
    ...
    if (course.assessment.type === 'quiz' || course.assessment.type === 'both') {
  ...
  try {
    ...
    if (assessmentType === 'quiz' && answers) {
      answers.forEach((answer, index) => {
        if (course.assessment.questions[index] && 
    ...
    user.myLiveCourses[userCourseIndex].userProgress.assessmentAttempts.push(attempt);
    
    // Update best score
    if (score > (user.myLiveCourses[userCourseIndex].userProgress.bestAssessmentScore || 0)) {
      user.myLiveCourses[userCourseIndex].userProgress.bestAssessmentScore = score;
    }
    ...
    res.json({
      ...
      attemptsRemaining: (course.assessment.retakesAllowed + 1 || 2) - 
        user.myLiveCourses[userCourseIndex].userProgress.assessmentAttempts.length
    });
