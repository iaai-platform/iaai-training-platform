...
const inPersonCourseSchema = new mongoose.Schema({
  ...
  venue: {
    ...
    type: { 
      type: String, 
      enum: ['hospital', 'training-center', 'clinic', 'university', 'conference-center'],
...
inPersonCourseSchema.virtual('displayDuration').get(function() {
  if (this.schedule.endDate && this.schedule.startDate) {
    const days = Math.ceil((this.schedule.endDate - this.schedule.startDate) / (1000 * 60 * 60 * 24)) + 1;
...
inPersonCourseSchema.virtual('isMultiDay').get(function() {
  if (this.schedule.endDate && this.schedule.startDate) {
    return this.schedule.endDate.getTime() !== this.schedule.startDate.getTime();
  }
  return false;
});
...
inPersonCourseSchema.virtual('instructorNames').get(function() {
  ...
  if (this.instructors.additional) {
    names.push(...this.instructors.additional.map(i => i.name).filter(Boolean));
  }
  return names.join(', ');
});
...
inPersonCourseSchema.pre('save', async function(next) {
  if (this.isModified('instructors')) {
    try {
      ...
      if (this.instructors.primary?.instructorId && !this.instructors.primary.name) {
        const instructor = await Instructor.findById(this.instructors.primary.instructorId)
          .select('firstName lastName fullName');
        if (instructor) {
          this.instructors.primary.name = instructor.fullName || 
        ...
        for (let i = 0; i < this.instructors.additional.length; i++) {
          ...
          if (inst.instructorId && !inst.name) {
            const instructor = await Instructor.findById(inst.instructorId)
              .select('firstName lastName fullName');
            if (instructor) {
              this.instructors.additional[i].name = instructor.fullName || 
                `${instructor.firstName} ${instructor.lastName}`;
            }
          }
        }
  ...
  if (this.isModified('certification.issuingAuthorityId') && 
  this.certification?.issuingAuthorityId) {
try {
  const CertificationBody = mongoose.model('CertificationBody');
  const body = await CertificationBody.findById(this.certification.issuingAuthorityId)
    .select('companyName');
  if (body) {
...
inPersonCourseSchema.methods.getLeadInstructor = async function() {
  if (this.instructors.primary?.instructorId) {
    return await mongoose.model('Instructor').findById(this.instructors.primary.instructorId);
  }
  return null;
};
...
inPersonCourseSchema.methods.getAttendancePercentage = function(userId) {
  ...
  const totalHours = userRecords.reduce((sum, record) => sum + (record.hoursAttended || 0), 0);
...
inPersonCourseSchema.methods.isCertificateEligible = function(userId) {
  ...
  const meetsAttendance = attendancePercentage >= this.certification.requirements.minimumAttendance;
...
inPersonCourseSchema.methods.removeFile = function(category, url) {
  ...
  if (category === 'mainImage') {
    ...
    this.media[category] = this.media[category].filter(fileUrl => fileUrl !== url);
  }
...
inPersonCourseSchema.statics.findAvailable = function() {
  return this.find({
    'basic.status': 'open',
    'schedule.startDate': { $gte: new Date() },
    $expr: { $lt: ['$enrollment.currentEnrollment', '$enrollment.seatsAvailable'] }
  });
};
...
module.exports = mongoose.models.InPersonAestheticTraining || 
