...
router.get(
  "/get-exam-answers/:courseId/:videoId",
  isAuthenticated,
  async (req, res) => {
    try {
      const { courseId, videoId } = req.params;
      console.log(
        `📊 Fetching exam answers - Course: ${courseId}, Video: ${videoId}`
      );
      ...
      const course = user.mySelfPacedCourses.find(
      ...
      const formattedAnswers = examResponse.answers.map((userAnswer) => {
        const question = video.exam.find(
...
router.get("/course-status/:courseId", isAuthenticated, async (req, res) => {
  try {
    ...
    const completedVideos =
      course.userCourseStatus?.completedVideos?.length || 0;
    const videosWithExams =
      course.videos?.filter((v) => v.exam && v.exam.length > 0) || [];
    const completedExams = course.userCourseStatus?.completedExams?.length || 0;

    const isCompleted =
      course.userCourseStatus?.userCourseTotalstatus === "Completed";
...
router.post("/reset-video-progress", isAuthenticated, async (req, res) => {
  try {
    ...
    if (course.videoProgress) {
      const progressIndex = course.videoProgress.findIndex(
    ...
    if (course.userCourseStatus?.completedVideos) {
      course.userCourseStatus.completedVideos =
        course.userCourseStatus.completedVideos.filter(
...
router.post("/retake-exam", isAuthenticated, async (req, res) => {
  try {
    ...
    if (course.userCourseStatus?.completedExams) {
      course.userCourseStatus.completedExams =
        course.userCourseStatus.completedExams.filter(
          (id) => id.toString() !== videoId
        );
    }
    ...
    if (course.userCourseStatus?.examResponses) {
      course.userCourseStatus.examResponses =
        course.userCourseStatus.examResponses.filter(
    ...
    const videosWithExams = course.videos.filter(
      (v) => v.exam && v.exam.length > 0
    );
    const allExamsCompleted =
      videosWithExams.length === 0 ||
      videosWithExams.every((v) =>
        course.userCourseStatus.completedExams.includes(v._id.toString())
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `🏆 Manual certificate generation requested - User: ${userId}, Course: ${courseId}`
      );
      ...
      // Find the course in user's self-paced courses
      const userCourse = user.mySelfPacedCourses.find(
        (c) => c.courseId.toString() === courseId
      );
      if (!userCourse) {
        return res
      ...
      if (existingCertificate) {
        return res.json({
          ...
          certificate: {
            ...
            verificationCode:
              existingCertificate.certificateDetails.verificationCode,
      ...
      try {
        ...
        const fakeRes = {
          status: (code) => ({
            json: (data) => {
              certificateResult = { status: code, data };
            },
          }),
          json: (data) => {
            certificateResult = { status: 200, data };
          },
        };
        ...
        if (
          certificateResult &&
          certificateResult.status === 200 &&
          certificateResult.data.success
        ) {
          return res.json({
            success: true,
  ...
  async (req, res) => {
    try {
      ...
      const user = await User.findById(userId).select(
        "myCertificates achievementSummary firstName lastName email"
      );
      if (!user) {
        return res
          .status(404)
