...
exports.getCourseDetails = async (req, res) => {
  try {
    ...
    if (course.instructors?.primary?.instructorId) {
      ...
      console.log('📌 Primary instructor ID:', course.instructors.primary.instructorId);
    }
    
    // Collect additional instructor IDs
    if (course.instructors?.additional && course.instructors.additional.length > 0) {
    ...
    if (instructorIds.length > 0) {
      console.log('🔍 Fetching instructor details for', instructorIds.length, 'instructors');
      ...
      if (course.instructors?.primary?.instructorId) {
        const primaryInstructor = instructors.find(inst => 
          inst._id.toString() === course.instructors.primary.instructorId.toString()
        );
        
        if (primaryInstructor) {
          courseWithFullDetails.instructorDetails.primary = primaryInstructor;
          console.log('✅ Primary instructor found:', primaryInstructor.firstName, primaryInstructor.lastName);
        } else {
      ...
      if (course.instructors?.additional && course.instructors.additional.length > 0) {
        courseWithFullDetails.instructorDetails.additional = course.instructors.additional
          .map(add => {
            const instructor = instructors.find(inst => 
              inst._id.toString() === add.instructorId.toString()
            );
            if (instructor) {
        ...
        console.log('✅ Additional instructors mapped:', courseWithFullDetails.instructorDetails.additional.length);
      }
    } else {
    ...
    if (course.certification?.issuingAuthorityId) {
      console.log('📌 Certification body ID:', course.certification.issuingAuthorityId);
      
      try {
        const certBody = await CertificationBody
          .findById(course.certification.issuingAuthorityId)
          .lean();
    ...
    courseWithFullDetails.enhancements = {
      availableSeats: (course.enrollment?.seatsAvailable || 0) - (course.enrollment?.currentEnrollment || 0),
      enrollmentPercentage: course.enrollment?.seatsAvailable > 0 ? 
        Math.round(((course.enrollment?.currentEnrollment || 0) / course.enrollment.seatsAvailable) * 100) : 0,
      daysUntilStart: course.schedule?.startDate ? 
        Math.ceil((new Date(course.schedule.startDate) - new Date()) / (1000 * 60 * 60 * 24)) : null,
      isUpcoming: course.schedule?.startDate && new Date(course.schedule.startDate) > new Date(),
    ...
    courseWithFullDetails.instructorNames = (() => {
      ...
      if (courseWithFullDetails.instructorDetails.primary) {
        ...
        names.push(primary.fullName || `${primary.firstName} ${primary.lastName}`);
      }
      
      if (courseWithFullDetails.instructorDetails.additional && courseWithFullDetails.instructorDetails.additional.length > 0) {
    ...
    console.log('- Has primary instructor details:', !!courseWithFullDetails.instructorDetails.primary);
    console.log('- Additional instructors count:', courseWithFullDetails.instructorDetails.additional.length);
    console.log('- Has certification body details:', !!courseWithFullDetails.certificationBodyDetails);
