...
exports.manageInstructors = async (req, res) => {
  try {
    ...
    console.log(
      `📋 Loading instructor management page. Found ${instructors.length} instructors`
    );
...
exports.getInstructors = async (req, res) => {
  try {
    const instructors = await Instructor.find({ isDeleted: false })
      .select(
        "firstName lastName email expertise status profileImage designation assignedCourses"
      )
      .lean();
...
exports.getActiveInstructors = async (req, res) => {
  try {
    ...
    console.log(
      `✅ API: Fetched ${formattedInstructors.length} active instructors`
    );
...
exports.createInstructor = async (req, res) => {
  try {
    ...
    // Handle file upload
    if (req.file) {
      instructorData.profileImage = req.file.path; // Cloudinary URL
      console.log(
    ...
    if (existing) {
      ...
      if (req.file && req.file.public_id) {
        try {
          ...
          console.log("🗑️ Deleted uploaded image from Cloudinary");
        } catch (deleteError) {
...
exports.updateInstructor = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (
        existingInstructor.profileImage &&
        existingInstructor.profileImage.includes("cloudinary")
      ) {
        try {
...
exports.deleteInstructor = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      instructor.profileImage.includes("cloudinary")
    ) {
      try {
        const publicId = extractPublicIdFromUrl(instructor.profileImage);
        if (publicId) {
          await cloudinary.uploader.destroy(publicId);
          console.log("🗑️ Deleted profile photo from Cloudinary:", publicId);
        }
      } catch (deleteError) {
...
exports.syncInstructorCourses = async (req, res) => {
  try {
    ...
    for (const course of inPersonCourses) {
      ...
      if (
        course.instructors.primary?.instructorId?.toString() === instructorId
      ) {
      ...
      if (
        course.schedule.startDate <= now &&
        (!course.schedule.endDate || course.schedule.endDate >= now)
      ) {
        status = "In Progress";
    ...
    for (const course of selfPacedCourses) {
      instructor.assignedCourses.push({
        ...
        status:
          course.basic.status === "published" ? "In Progress" : "Upcoming",
      });
    }
    ...
    console.log(
      `✅ Synced ${instructor.assignedCourses.length} courses for ${instructor.email}`
    );
...
exports.assignCourse = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Course ${courseData.courseTitle} assigned to ${instructor.email}`
    );
...
exports.updateCourseStatus = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Updated course status to ${status} for instructor ${instructor.email}`
    );
...
exports.getInstructorStats = async (req, res) => {
  try {
    ...
    const stats = {
      ...
      coursesByRole: {
        lead: instructor.assignedCourses.filter(
          (c) => c.role === "Lead Instructor"
        ).length,
        coInstructor: instructor.assignedCourses.filter(
          (c) => c.role === "Co-Instructor"
        ).length,
        assistant: instructor.assignedCourses.filter(
          (c) => c.role === "Assistant"
        ).length,
      },
      ratings: {
...
function extractPublicIdFromUrl(url) {
  try {
    ...
    const folderIndex = parts.indexOf("iaai-platform");
    if (folderIndex !== -1) {
      return `iaai-platform/instructors/${publicId}`;
    }
  ...
  try {
    const instructor = await Instructor.findOne({
      $or: [
        ...
        {
          $expr: {
            $eq: [{ $concat: ["$firstName", " ", "$lastName"] }, identifier],
          },
        },
      ],
      isDeleted: false,
...
exports.checkInstructorAvailability = async (
  instructorId,
  startDate,
  endDate
) => {
