...
certificationBodySchema.virtual("displayName").get(function () {
  return this.shortName
    ? `${this.companyName} (${this.shortName})`
    : this.companyName;
});

// Method to add certification statistics
certificationBodySchema.methods.updateCertificationStats = function (
  increment = 1
) {
...
certificationBodySchema.methods.canIssueCertification = function (
  certificationType
) {
  return (
    this.isActive &&
...
certificationBodySchema.statics.findBySpecialization = function (
  specialization
) {
  return this.find({
    isDeleted: false,
...
certificationBodySchema.pre("save", function (next) {
  if (!this.shortName && this.companyName) {
    ...
    const words = this.companyName
      .split(" ")
      .filter(
        (word) =>
          !["of", "and", "for", "the", "in", "on", "at"].includes(
            word.toLowerCase()
          )
      );
...
module.exports =
  mongoose.models.CertificationBody ||
  mongoose.model("CertificationBody", certificationBodySchema);
